...
test('getMessages con since filtra messaggi più vecchi', () => {
  ...
  sqlite.prepare('INSERT INTO messages (author, content, created_at) VALUES (?, ?, ?)').run('Mario', 'Vecchio', oldTs)
  sqlite.prepare('INSERT INTO messages (author, content, created_at) VALUES (?, ?, ?)').run('Nicol', 'Nuovo', newTs)
